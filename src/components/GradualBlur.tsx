  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        // Start from 0: (2^(p*4) - 1) / 15 * strength
      ...
      const p1 = Math.max(0, math.round((increment * i - increment * 1.5) * 10) / 10);
      const p2 = math.round(increment * i * 10) / 10;
      const p3 = math.round((increment * i + increment * 0.5) * 10) / 10;
      const p4 = math.round((increment * i + increment * 2) * 10) / 10;

      let gradient = `transparent ${p1}%, black ${p2}%`;
      if (p3 <= 100) gradient += `, black ${p3}%`;
      // For the last div, we want it to stay black until the end
      if (i < config.divCount && p4 <= 100) gradient += `, transparent ${p4}%`;
      ...
      const divStyle: CSSProperties = {
        ...
        transition:
          config.animated && config.animated !== 'scroll'
            ? `backdrop-filter ${config.duration} ${config.easing}, background-color ${config.duration} ${config.easing}`
            : undefined
      };
